...
def userinteraction(scan):
    global userId, subCamDepth, userTime, userTimeDelta, pubSound, state
    
    
    if (time() - userTime) > userTimeDelta:
        if userId == 0 and state == "START":
        	#subCamDepth = rospy.Subscriber("/camera/depth/points", PointCloud2, callback);
        	userId = scan.user
            ...
            #subCamDepth.unregister()
            userId = 0
            rospy.loginfo("user logged out")
            
            sr = SoundRequest()     	
            sr.sound = -3 #Say text        	
            sr.command = 1 #Play once            
            sr.arg = "Thank you for shopping with us Mr. Keller."
            pubSound.publish(sr)       
...
def initialization():
    ...
    pubSound = rospy.Publisher('/robotsound', SoundRequest, queue_size=10)
    subScanUser = rospy.Subscriber("scan_user", Scan_User, userinteraction);
