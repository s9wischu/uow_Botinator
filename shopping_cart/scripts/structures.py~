...
class Item:
    ...
    def writeXML(self, parent): 
        ...
        nodeLocationX = xml.SubElement(node, "locationX")
        nodeLocationX.text = str(self.getLocation()[0])
        nodeLocationY = xml.SubElement(node, "locationY")
        nodeLocationY.text = str(self.getLocation()[1])
    ...
    def fromXML(node): 
        ...
        x = int(float(node.find("locationX")))
        y = int(float(node.find("locationY")))
    ...
    def getHTMLListString(self):
        ...
        root = html.parse("/home/motionlab/Desktop/git/Botinator/shopping_cart/scripts/html/product_list_item.html").getroot()
